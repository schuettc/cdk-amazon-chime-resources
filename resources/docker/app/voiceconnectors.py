...
def build_logging(voice_connector_id, sip_logs, media_metric_logs):
    logger.info(f"Putting Voice Connector Logging Configuration")
    logger.info(f"SIP Logs: {sip_logs}")
    logger.info(f"Media Metric logs: {media_metric_logs}")
    time.sleep(10)
    if sip_logs == "false":
        sip_logs = False
    else:
    ...
    if media_metric_logs == "false":
        media_metric_logs = False
    else:
        media_metric_logs = True

    try:
        chime.put_voice_connector_logging_configuration(
            VoiceConnectorId=voice_connector_id,
            LoggingConfiguration={"EnableSIPLogs": sip_logs, "EnableMediaMetricLogs": media_metric_logs},
        )
...
def create_voiceconnector(
    ...
    **kwargs,
    ...
    if logging:
        ...
        build_logging(voice_connector_id, logging["sipLogs"], logging["mediaMetricLogs"])

    return voice_connector_id


def delete_voiceconnector(uid):
